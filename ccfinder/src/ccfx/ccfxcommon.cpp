...
{
#if defined _OPENMP
	return (boost::format("max worker threads/available OpenMP threads: %d/%d") % maxWorkerThreads % omp_get_max_threads()).str();
#else
	return (boost::format("max worker threads/available OpenMP threads: %d/0") % maxWorkerThreads).str();
#endif
}
